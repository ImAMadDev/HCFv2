...
class Texts
{
    ...
    public function onTick(): void
    {
        foreach ($this->main->getServer()->getOnlinePlayers() as $player) {
            if (!in_array($player->getName(), array_keys($this->send)) && stripos($player->getRegion()->get(), "Spawn") !== false) {
                ...
                $message = str_replace("{crate_subtext}", "&7Left click without key to see rewards\n&7Right click with key to open crate", $message);
                FloatingTextApi::sendText($this->textID, $player, $message);
                $this->send[$player->getName()] = $player;
            }
        }
    ...
    {
        $players = [];
        foreach ($this->position->getWorld()->getNearbyEntities(new AxisAlignedBB($this->position->getFloorX() - $distance, $this->position->getFloorY() - $up, $this->position->getFloorZ() - $distance, $this->position->getFloorX() + $distance, $this->position->getFloorY() + $up, $this->position->getFloorZ() + $distance)) as $e) {
